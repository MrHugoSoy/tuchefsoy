...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const { data: recipe } = await supabase.from('recipes').select('title, description, image_url, category').eq('slug', slug).single()
...
export default async function RecipeSlugPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data: recipe, error } = await supabase.from('recipes').select('*, author:profiles(*)').eq('slug', slug).single()
  if (error || !recipe) notFound()

  const r = recipe as Recipe
  const id = String(r.id)

  await supabase.from('recipes').update({ views_count: (r.views_count ?? 0) + 1 }).eq('id', r.id)

  const { data: { user } } = await supabase.auth.getUser()
  let initialLiked = false
  if (user) {
    const { data: likeRow } = await supabase.from('recipe_likes').select('id').eq('recipe_id', r.id).eq('user_id', user.id).single()
    initialLiked = !!likeRow
  }

  const { data: relatedData } = await supabase
    .from('recipes')
    .select('id, slug, title, image_url, youtube_url, prep_time, cook_time, difficulty, category, likes_count, author:profiles(username, full_name, avatar_url)')
    .eq('category', r.category).neq('id', r.id).order('likes_count', { ascending: false }).limit(4)
  ...
  const ResumenContent = (
    <div className="flex flex-col gap-4">
      {(r.rating_count ?? 0) > 0 && (
        <div className="flex items-center justify-between">
          <span className="text-sm text-muted flex items-center gap-1">
            <svg className="w-4 h-4 text-[#f59e0b]" fill="#f59e0b" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" /></svg>
            Calificación
          </span>
      ...
      <div className={`flex items-center justify-between ${(r.rating_count ?? 0) > 0 ? 'border-t border-border pt-4' : ''}`}>
        <span className="text-sm text-muted">Tiempo total</span><span className="text-sm font-semibold">{totalTime} min</span>
      </div>
      <div className="flex items-center justify-between border-t border-border pt-4">
        <span className="text-sm text-muted">Preparación</span><span className="text-sm font-semibold">{r.prep_time} min</span>
      </div>
      <div className="flex items-center justify-between border-t border-border pt-4">
        <span className="text-sm text-muted">Cocción</span><span className="text-sm font-semibold">{r.cook_time} min</span>
      </div>
      <div className="flex items-center justify-between border-t border-border pt-4">
        <span className="text-sm text-muted">Porciones</span><span className="text-sm font-semibold">{r.servings}</span>
      </div>
      <div className="flex items-center justify-between border-t border-border pt-4">
      ...
      {!youtubeId && (
        <div className="flex items-center justify-between border-t border-border pt-4">
          <span className="text-sm text-muted">Ingredientes</span><span className="text-sm font-semibold">{r.ingredients.length}</span>
        </div>
      )}
      <div className="flex items-center justify-between border-t border-border pt-4">
        <span className="text-sm text-muted flex items-center gap-1">
  ...
  return (
    <>
      ...
      <style dangerouslySetInnerHTML={{ __html: `@media print { header, footer, nav, .no-print { display: none !important; } body { background: white !important; color: black !important; font-size: 10pt; } @page { margin: 1cm; } }` }} />
          ...
          <div>
            <div className="mb-6">
              <div className="flex items-center gap-2 mb-3 no-print flex-wrap">
                ...
                {r.tags?.map((tag) => <span key={tag} className="px-2.5 py-1 text-xs bg-[#f7f7f7] rounded-full text-muted">{tag}</span>)}
              </div>
            ...
            {r.author && (
              <div className="flex items-center gap-3 mb-8 pb-8 border-b border-border">
                <div className="w-10 h-10 rounded-full bg-border overflow-hidden shrink-0 no-print">
                  {r.author.avatar_url ? <Image src={r.author.avatar_url} alt={r.author.username} width={40} height={40} className="object-cover" /> : <div className="w-full h-full flex items-center justify-center text-sm font-semibold text-muted">{r.author.username[0].toUpperCase()}</div>}
                </div>
                <div>
                  <Link href={`/perfil/${r.author.username}`} className="text-sm font-medium hover:text-brand transition-colors">{r.author.full_name ?? r.author.username}</Link>
            ...
            {!youtubeId && r.ingredients.length > 0 && (
        ...
        {related.length > 0 && (
          <div className="mt-16 pt-10 border-t border-border no-print">
            <div className="flex items-center justify-between mb-6">
              ...
              <Link href={`/?category=${encodeURIComponent(r.category)}`} className="text-sm text-brand hover:underline font-medium">Ver más de {r.category}</Link>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {related.map((rel) => {
                const relTime = rel.prep_time + rel.cook_time
                const relYtId = rel.youtube_url ? getYoutubeId(rel.youtube_url) : null
                const relHref = rel.slug ? `/receta/${rel.slug}` : `/recipe/${rel.id}`
                return (
                  <Link key={rel.id} href={relHref} className="group block rounded-[12px] border border-[#f0f0f0] bg-white overflow-hidden hover:shadow-md transition-shadow">
                    <div className="relative aspect-[4/3] bg-[#f7f7f7] overflow-hidden">
                      {rel.image_url ? <Image src={rel.image_url} alt={rel.title} fill className="object-cover group-hover:scale-105 transition-transform duration-300" sizes="(max-width: 768px) 50vw, 25vw" /> : <div className="absolute inset-0 flex items-center justify-center text-[#d0d0d0]"><svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14" /></svg></div>}
                      {relYtId && <div className="absolute inset-0 flex items-center justify-center"><div className="w-8 h-8 bg-black/60 rounded-full flex items-center justify-center"><svg className="w-4 h-4 text-white ml-0.5" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg></div></div>}
                    </div>
                    <div className="p-3">
                      <h3 className="font-semibold text-[#111] text-sm leading-snug mb-2 line-clamp-2">{rel.title}</h3>
                      <div className="flex items-center gap-2 text-xs text-[#737373]">
                        <span className="flex items-center gap-1"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>{relTime} min</span>
